...
for i in xrange(100):
   ...
   if i%10 == 0 and i!=0:
      ...
      acc = sess.run([accuracy], feed_dict={x: test[0], y_: yTest, keep_prob: 1.0})
   ...
   cross_entropyD, y_convD, summary_str = sess.run([cross_entropy, y_conv, summary_op], feed_dict={x: batch[0], y_: yTrain, keep_prob: 1})
   f1.write(str(cross_entropyD)+"\n")
   f1.write(str(np.argmax(y_convD, axis=1))+"\n")
   f1.flush()
